...
The data comes with package `afex`, so we can load it right away.
...
For this and the following exercises use the `fhch` `data.frame` (i.e., the data after removing errors) as the initial data.
...
- The pipe `%>%` allows to concatenate calls from left to right.
...
`summarise` allows to use more than one aggregation function. Extend the previous code and also calculate the standard deviation, `sd()`, per task. Does the task that is faster also has th elower variability in RT (i.e., smaller sd)?
