...
Research question: Does the Equation (i.e., *P*(if *A* then *C*) = *P*(*C*|*A*)) hold? 

Full design: 4 x 3. Both factors repeated-measures. Here we ignore

For each item, participants provide idiosyncratic estimates of *P*(if *A* then *C*) (`dv`) and *P*(*C*|*A*) (`c_given_a`).  
Each participant worked on 12 items, 12 estimates of *P*(*C*|*A*) (`c_given_a`).

For now, we ignore all other factors!

Data available in the folder (also at: https://osf.io/j4swp/)

## Exercise 1: Analyse the data using the no-pooling approach.
- Calculate the regression between *P*(if *A* then *C*) and *P*(*C*|*A*) separately for each participant.
...
- `rel_cond`: relevance condition with three levels. Within-subject variable (can be ignored)
...
Before the next step, let's take a look at the data. It suggests indeed that the relationship between IV and DV. But does it hold for the participants?
...
- Your task is to calculate the regression parameter (i.e., slope, potentially also the intercept) for each participant (i.e., relationship of `dv` and `c_given_a` for each `p_id`).
