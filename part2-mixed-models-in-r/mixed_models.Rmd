...
par(pty="s")
limits <- c(-0.5, 0.5)
plot(dv ~ c_given_a, datr, asp = 1, ylim=limits, xlim=limits)
abline(m_fixed)
```

$$y=\beta_0 + \beta_{C|A}X_{C|A} + \epsilon$$
...
$$y=\beta_0 + (\beta_{C|A}+ S_{C|A})X_{C|A} + \epsilon$$
...
- In `lme4::lmer` syntax: `lmer(dv ~ c_given_a + (0+c_given_a|p_id), datr)`
...
m_tmp <- lmer(dv ~ c_given_a + (0+c_given_a|p_id), datr)
rnd_coefs <- coef(m_tmp)$p_id
par(pty="s")
limits <- c(-0.5, 0.5)
plot(dv ~ c_given_a, datr, 
...
$$y=\beta_0 + S_0 + (\beta_{C|A}+ S_{C|A})X_{C|A} + \epsilon$$
...
Each $S_i$ idiosyncratic intercept & slope: `lmer(dv ~ c_given_a + (c_given_a|p_id), datr)`

```{r, echo=FALSE, dpi=300, fig.width=3.5, fig.height=3.5, warning=FALSE, out.width='25%'}
m_tmp <- lmer(dv ~ c_given_a + (1+c_given_a|p_id), datr)
rnd_coefs <- coef(m_tmp)$p_id
rnd_coefs <- coef(m_tmp)$p_id
par(pty="s")
limits <- c(-0.5, 0.5)
...
ggplot(datr, aes(y = dv, x = c_given_a)) +
  geom_point() +
  facet_wrap(~ rel_cond) + 
...
df_gravity <- as.data.frame(summary(emmeans::emtrends(m_fixed, "rel_cond", var = "c_given_a")))
...
```{r}
mmach <- lm(score ~ Machine, Machines)
...
```{r, warning=FALSE}
m2 <- lmer(score~Machine+(Machine|Worker), Machines)
...
```{r}
emm_options(lmer.df = "asymptotic") # or "Kenward-Roger" or "Satterthwaite"
emtrends(m_full, "rel_cond", var = "c_given_a")
```
...
```{r, message=FALSE}
data("Machines", package = "MEMSS")
mmach <- mixed(score~Machine+(Machine|Worker), Machines)
mmach
```
