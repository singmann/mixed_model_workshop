...
- `tidyverse`: Selection of packages curated/developed by `RStudio`:
    ...
    - [`purr`](http://purrr.tidyverse.org/) and [`broom`](https://broom.tidyverse.org/): Advanced modeling with the `tidyverse`.
...
Try replacing `summary(cars)` with `knitr::kable(summary(cars))`

---
class: small, inline-grey

- visit: [`https://github.com/singmann/mixed_model_workshop`](https://github.com/singmann/mixed_model_workshop)

![](figures/github-workshop.png)
...
- In each folder:
  ...
  - You can follow the presentation by following this file. Don't forget: run a code chunk with `Ctrl`/`Cmd` + `Shift` + `Enter`
  - `.html` is the full presentation you are seeing. After opening, press `h` for help.
  - If available, `.R` file contains only the code, without additional slide text.
...
- To learn more about the `tidyverse` check out the free "R for Data Science" book by Wickham & Grolemund
...
- Open `exercises/exercise_0.Rmd`
- Follow text and try to solve a few small tasks helping you to get comfortable with the `tidyverse`.
- Main goal is getting comfortable with `dplyr` and `ggplot2` syntax.
