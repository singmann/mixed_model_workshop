...
# `emmeans` for Follow-Up/Post-Hoc Tests

.pull-left[
```{r, message=FALSE, warning=FALSE}
library("emmeans")      # or: library("emmeans")
emmeans(m6, ~education) # or: emmeans(m6, ~education)
```
...
.pull-right[
```{r, message=FALSE}
pairs(emmeans(m6,~education),adjust='holm')
```
]
...
- Requires considerable data on the individual level
...
- Estimation can be difficult in a frequentist setting (therefore alternatively penalized or restricted maximum likelihood estimation)
...
## Partial-Pooling: Repeated-measures ANOVA
